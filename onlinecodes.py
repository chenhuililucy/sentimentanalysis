...
def parse(file1, file2):
    ...
    for line in hand:
        ...
        if re.findall('^COMPANY CONFORMED NAME:',line):
            ...
            break
            #Firstly remove leading and trialing character of the line. 
            #Find ":", start new line after 
            #Link conformed name togerther with header and pass string of company unique name, new line 
    hand=open(file1)
    for line in hand:
        line=line.strip()
    ...
    with open(file2, 'a') as f:
        ...
        f.close()
    hand.close()
    #what I dont understand is the relationshiop between the file1 and file2
...
'''
filepath="/Users/lucy/Desktop/down"   

###############################################################################
#This is the master download file that include all of the links to SEC filings.
###############################################################################
download=os.path.join(filepath,"downloadlist.csv")
...
with open(download, 'r') as txtfile:
    ...
    for line in reader:
        ...
        url = 'https://www.sec.gov/Archives/' + line[3].strip()
