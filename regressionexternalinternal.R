...
reg = lm_robust(as.double(roac1) ~ as.double(internal)+as.double(external)+log(length)+log(as.double(BOV))+gic+as.double(roac), data = subset(`vectorfinal(8)`))
summary(reg)

reg = lm_robust(as.double(roac1) ~ as.double(intext)+log(length)+log(as.double(BOV))+gic+as.double(roac), data = subset(`vectorfinal(8)`))
summary(reg) #???
